...
def balance_resources(drcs, receiver_drc, donor_drc, rtom, transfer_value):
    ...
    :param resource_tracker : Tracker of resources to keep track of the resources that have been transferred.
    :return: Updated DRCs with case IDs after balancing.
    :before tranfering and balancing check recources from the donner_drc , reciever_drcgo below 20% 
        example : Initial DRCs with Case IDs:
                    D1:
    ...
    for case_id, (drc, resource) in drcs.items():
        ...
        # print(f"Collected Bunch of Resources from database : {resource_tracker}") ~for test
