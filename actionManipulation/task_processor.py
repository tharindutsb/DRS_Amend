...
def process_single_batch(task):
    ...
    try:
        ...
        success, template_task = fetch_and_validate_template_task(template_task_collection, template_task_id, task_type)
        if not success:
            raise Exception(template_task)

        # Step 3: Fetch transaction details
        transaction_collection = get_collection("Case_distribution_drc_transactions")
        success, amend_details = fetch_transaction_details(transaction_collection, case_distribution_batch_id)
        if not success:
            raise Exception(amend_details)

        # Step 4: Fetch cases for the batch
        case_collection = get_collection("DRS.Tmp_Case_Distribution_DRC")
        success, cases = fetch_cases_for_batch(case_collection, case_distribution_batch_id)
        if not success:
        ...
        for distribution in array_of_distributions:
            ...
            success, updated_drcs = balance_resources(drcs, receiver_drc, donor_drc, rtom, transfer_value)
            if not success:
                raise Exception(updated_drcs)

            # Step 7: Update case distribution collection
            success, original_states = update_case_distribution_collection(case_collection, updated_drcs, existing_drcs)
            if not success:
                raise Exception(original_states)

            # Step 8: Update summary in MongoDB
            summary_collection = get_collection("DRS_Database.Case_Distribution_DRC_Summary")
            success, original_counts = update_summary_in_mongo(summary_collection, transaction_collection, updated_drcs, case_distribution_batch_id)
            if not success:
                ...
                raise Exception(original_counts)
